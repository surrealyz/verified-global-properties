...
class ILPModel(object):
    ...
    def global_attack(self, cid_list, prop_name, *args):
        ...
        fi_list = args[0]
        print('\n*** feature index %s, cid list %s ***' % (fi_list, cid_list))
                            ...
                            if p_negate == False:
                                ...
                                if weight != 0:
                                    if tup not in self.pcheck:
                                        ...
                                        if self.default_lo != None and feat_idx != 7 and threshold <= self.default_lo:
        ...
        leaf_values = [self.leaf_value[cid] for cid in cid_list]
        first_llist = [self.L[cid] for cid in cid_list]
        second_llist = [self.L[cid+self.cln_model.last_cid+1] for cid in cid_list]
        ...
        if status == GRB.OPTIMAL:
            ...
            for key in self.pdict.keys():
                ...
                if type(key) != str:
                    ...
                    if key != 7:
                        ...
                        val = self.genval(self.pdict[key], isint, min_val = -300)
                    #val = self.genval(self.pdict[key], isint)
                    x1[idx] = val
                    x2[idx] = val
                else:
            ...
            print('\n*** counterexample ***')
            print('x1:', x1)
            print('weighted_sum_of_leaves:', sum([self.leaf_value[lid]*self.L[lid].x for lid in cid_list]))
            print('x2:', x2)
            print('weighted_sum_of_leaves:', sum([self.leaf_value[cid+self.cln_model.last_cid+1]*self.L[cid+self.cln_model.last_cid+1].x for cid in cid_list]), '\n')
            ...
            atom_1, clause_1, leaf_ret_1 = check_states_select(self.cln_model, cid_list, self.int_indices, x1)
            atom_2, clause_2, leaf_ret_2 = check_states_select(self.cln_model, cid_list, self.int_indices, x2)
    ...
    def genval(self, tuple_list, isint, min_val = 0):
...
def main(args):
    ...
    #for j in range(1, 7):
    #    attack_model.attack('monotonicity', j, 1)
    ...
    if args.monotonicity != None:
        ...
        # twitter_spam/cln_models/social_honeypot_v10_20210413_all_v6.json
        cid_list = list(range(115, 179))
        for i, fi in enumerate(monotone_index_list): 
            direction = monotone_direction[i]
            attack_model.global_attack(cid_list, 'monotonicity', [fi], direction)
        ...
        for i, fi in enumerate(stable_index_list):
            attack_model.global_attack(cid_list, 'stability', [fi], stable_threshold)
    elif args.eps != None:
