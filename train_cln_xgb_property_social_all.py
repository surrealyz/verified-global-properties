...
from social_attack_ilp_select import MAX_INT32, ILPModel
...
def parse_args():
    ...
    parser.add_argument('--header', type=str, help='csv containing field names for features.', required=True)
...
def train_property(args, cln_model, start_cid, x_train, y_train, \
    ...
    # CEGAR loop to add constraint, and train CLN
    for epoch in range(args.epoch):
        ...
        all_cids = list(range(start_cid, cln_model.last_cid+1))
            ...
            for k, prop_param in enumerate([args.monotonicity, args.stability, \
                ...
                for i, fi in enumerate(prop_index_list):
                    if k == 0:
                        ...
                        attack_args = []
                        clause_args = []
                    changed_atoms, pred_scores, ret = oracle.global_attack(cid_list, prop_name, *attack_args)
        ...
        if cur_counter == 0:
            ...
            #cln_model.prettyprint()
            #print("CLN model with best validation AUC", auc, "saved to", cur_model_path, "\n")
            print("CLN model AUC", auc, "saved to", cur_model_path, "and", cln_model.save_json_path, "\n")
            cln_model.save_as_json()
            #print("model json with best validation AUC", auc, "saved to", cln_model.save_json_path)
...
def main(args):
    ...
    mono_cids = []
    stable_cids = []
    feat_subset = {'ex_mono': ex_mono, 'ex_lowcost': ex_lowcost}
    cid_choice = {'ex_mono': stable_cids, 'ex_lowcost': mono_cids}
    if args.randfree == True:
        ex_seq = ['ex_lowcost', 'ex_lowcost', 'ex_mono', 'ex_mono']
    ...
    new_start = cln_model.last_cid + 1
   
    for iteration in range(1, args.num_boost_round):
        # set dtrain and param accordingly
        if args.randfree == True:
            #if iteration <= args.num_boost_round - 3:
            if iteration <= args.num_boost_round - 2:
                # DEBUG
                #continue
                ex_name = ex_seq[iteration-1]
                # choose subset of features
                exfeat_list = feat_subset[ex_name]
        ...
        if args.add == 'tree':
            ...
            # figure out the list of cids for mono and stable
            if args.randfree == True:
                #if iteration <= args.num_boost_round - 3:
                if iteration <= args.num_boost_round - 2:
                    ex_name = ex_seq[iteration-1]
                    # choose cid list
                    cur_cid_list = cid_choice[ex_name]
                    cur_cid_list += list(range(new_start, cln_model.last_cid+1))
                else:
                    # accumulate both mono_cids and stable_cids
                    mono_cids += list(range(new_start, cln_model.last_cid+1))
                    stable_cids += list(range(new_start, cln_model.last_cid+1))
    ...
    if cln_model.best_model_path != None:
        ...
        best_model.save_as_json()
        print("model json with best validation AUC save to", best_model.save_json_path)
        # for model_path in prev_saves:
        #     if model_path != cln_model.best_model_path:
        #         cmd = 'rm %s' % model_path
        #         print(cmd)
        #         os.system(cmd)
